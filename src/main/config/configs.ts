...
const config = {
    ...
    mail: {
        nodemailer: {
            ...
            auth: {
                user: process.env.NODEMAILER_USER || "scot.muller@ethereal.email",
                pass: process.env.NODEMAILER_PASSWORD || "NUqzF7Mf464kn39wkA"
            }
        },
        defaultSender: {
            name: {
                firstName: process.env.DEFAULT_MAIL_SENDER_FIRST_NAME || "gabriel",
                lastName: process.env.DEFAULT_MAIL_SENDER_LAST_NAME || "lopes"
            },
            address: process.env.DEFAULT_MAIL_SENDER_ADDRESS || "gabriel.lopes16@hotmail.com"
        }
    },
    imageStorage: {
