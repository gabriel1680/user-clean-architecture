...
import { UserDbConverter, UserDomainToDbConverter } from "@infra/database/helpers/user";


@EntityRepository(UserSQL)
class UserRepositoryTypeORM extends Repository<UserSQL> implements UserRepository
{
    public async exists(email: string): Promise<boolean> {
        const response = await this.query('SELECT CASE WHEN EXISTS (SELECT * FROM users WHERE email = $1) THEN ' +
            'true ELSE false END', [email]);
        return response[0].case;
    }

    public async existsById(id: string): Promise<boolean> {
        const response = await this.query('SELECT CASE WHEN EXISTS (SELECT * FROM users WHERE id = $1) THEN ' +
            'true ELSE false END', [id]);
        return response[0].case;
    }
    ...
    public async getRole(id: string): Promise<string> {
        const { role } = await this.query("SELECT role FROM users WHERE id=$1", [id]);
        return role;
    }

    public async findByConfirmLink(confirmLink: string): Promise<boolean> {
        const response = await this.query('SELECT CASE WHEN EXISTS (SELECT * FROM users WHERE confirm_link = $1)' +
            ' THEN true ELSE false END', [confirmLink]);
        return response[0].case;
    }

    public async removeConfirmLink(confirmLink: string): Promise<void> {
        await this.query("UPDATE users SET confirm_link=NULL WHERE confirm_link=$1", [confirmLink]);
    }
