...
export class Mailer implements MailService {
    ...
    public async send(addressees: Email[], message: MailMessage): Promise<void> {
        await this._nodeMailerTransporter.sendMail({
            ...
            html: message.body,
            attachments: !message.attachments ? null : this.buildAttachments(message)
        });
    }
    ...
    private buildAddresseesString(addressees: Email[]): string {
        ...
        addressees.map(addressee => {
            addresseesString.push(addressee.address);
        });
