import { AuthenticateUserController } from "../../../../src/adapters/presentation/controllers/user";
import { UserAuthenticator, UserTokenAuthenticator } from "../../../../src/application/usecases/user/Authenticate/Interfaces";
...
import { InvalidEmailOrPasswordError } from "../../../../src/application/usecases/user/Authenticate/Errors";
...
describe("Unit Tests Of Authenticate UserMongo Controller", () => {
	...
	it("should be able to call correctly the services", async () => {
		...
		const authenticateUserController = new AuthenticateUserController(fakeAuthenticator, fakeTokenAuthenticator);
		await authenticateUserController.handle(userData);

		expect(spy).toHaveBeenCalledTimes(1);
		return expect(spy).toHaveBeenCalledWith(userData.email, userData.password);
	});
	...
	it("should not be able to return user and token by receiving either invalid parameters", async () => {
		...
		const fakeTokenAuthenticator = new FakeTokenAuthenticator();
		// @ts-ignore
		fakeAuthenticator.execute = jest.fn(() => new InvalidEmailOrPasswordError());
		const authenticateUserController = new AuthenticateUserController(fakeAuthenticator, fakeTokenAuthenticator);
		const response = await authenticateUserController.handle(userData);
